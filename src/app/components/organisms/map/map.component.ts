...
import { SearchBarComponent } from '../search-bar/search-bar.component'; // Must be imported directly ?
import { LocateComponent } from '@components';
import { EventBusService, MapService } from '@services';
import { OverpassTurboResult } from '@interfaces';
...
export class MapComponent implements AfterViewInit, OnDestroy {
	...
	public ngAfterViewInit(): void {
		if (this.map) {
			...
			this.overpassTurboResultChangeSubscription = this.eventBusService
				.onOverpassTurboResultsChange()
				.subscribe((results: OverpassTurboResult[]) => {
					this.mapService.clearMarkers();
					results.forEach((result: OverpassTurboResult) => {
						if (result.type === 'node') {
							this.mapService.addMarker(
								result.geometry[0].lat,
								result.geometry[0].lng,
								result
							);
			...
			this.mapChangeSubscription = this.eventBusService
				.onMapChange()
				.subscribe(() => {
