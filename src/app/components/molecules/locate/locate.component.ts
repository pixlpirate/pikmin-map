...
export class LocateComponent {
	...
	public locate(): void {
		...
		map.locate({ setView: false, watch: true }).on(
			'locationfound',
			(event: LocationEvent) => {
				if (this.marker) {
					...
					this.marker
						.getPopup()
						?.setContent(this.getPopupContent(event.accuracy));
					...
					this.marker = marker(event.latlng, {
						...
						.addTo(map)
						.bindPopup(this.getPopupContent(event.accuracy));

					this.mapService.flyTo(event.latlng, 18);
				}
			}
		);
	}
