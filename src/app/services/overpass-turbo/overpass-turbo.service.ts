...
import { DecorService, EventBusService, MapService, ToastService } from '../../services';
...
export class OverpassTurboService
{
	...
	)
	{
		if ( !this.decorSubscription ) {
			this.decorSubscription = this.decorService.getDecors().subscribe( { next: ( decors: Decor[] ) => this.decors = decors } );
		}

		// Prepare the Overpass Turbo subject
		this.fetchOverpassTurboResultsSubject.pipe(
	...
	private fetchOverpassTurboResults$( decors: Decor[] ): Observable<any>
	{
		...
		this.toastService.add( {
			message: `<i class="loader icon icon-spin6"></i>${this.translate( "LOADING_OVERPASS" )}`,
			duration: Infinity,
			id: -1,
			type: "success"
		} );

		return this.http.get( `https://overpass-api.de/api/interpreter?data=${encodeURI( body )}` ).pipe(
			tap( ( response: any ) =>
			{
				...
				const elements = response.elements.filter( ( element: any ) => element.type === "node" || element.type === "way" );
				...
				this.eventBusService.setOverpassTurboResults(
					elements.map( ( element: any ) =>
					{
						return {
							id: element.id,
							name: element.tags.name ? element.tags.name : `${this.translate( "UNKNOWN_NAME" )}`,
							type: element.type,
							geometry: element.geometry
								? element.geometry.map( ( geo: { lat: number, lon: number; } ) => new LatLng( geo.lat, geo.lon ) )
	...
	public prepareOverpassTurboQuery( decors: Decor[] ): string
	{
		...
		{
			body += decor.area ? `area["ISO3166-1"="${decor.area}"]->.${decor.area};\n` : ``;
		} );
		...
		decors.forEach( ( decor: Decor ) =>
		{
			decor.tags.forEach( ( tag: string | string[] ) =>
			{
				...
				body += decor.area ? `(area.${decor.area})(${bbox});\n` : `(${bbox});\n`;
			} );
		} );
	...
	{
		const formattedTags = Object.keys( tags ).map( ( key: string ) => `${key}=${tags[ key ]}` );
		let decors: Decor[] = [];
