...
- 已在實例選擇器中新增自建 Overpass 鏡像，可在公共實例過載時作為備用。
- 修復取消選擇飾品後再次選擇時不再顯示結果的問題。
- 所選 Overpass 實例過載時，地圖現在會自動改用自建鏡像，並繼續使用成功回應的實例。
...
- 已在「關於」頁面新增自定義 Overpass 實例選擇器，並會在本地保存偏好的實例。

### v1.0.14 <time>2026-04-02</time>

- 感謝 Falldog 修復了一個導致訊息無限顯示的問題，並新增了一項功能以減少傳送到 Overpass Turbo 的請求數量！
- 已將 Overpass Turbo 實例切換為法國實例，相較於德國實例稍微快一些。🐔

### v1.0.13 <time>2025-12-16</time>

- 感謝 _cyanliu_ 修正了便利商店中的一個拼寫錯誤，該錯誤導致圖標無法顯示！
