...
- 已在实例选择器中新增自建 Overpass 镜像，可在公共实例过载时作为备用。
- 修复取消选择饰品后再次选择时不再显示结果的问题。
- 所选 Overpass 实例过载时，地图现在会自动改用自建镜像，并继续使用成功响应的实例。
...
- 已在「关于」页面新增自定义 Overpass 实例选择器，并会在本地保存偏好的实例。

### v1.0.14 <time>2026-04-02</time>

- 感谢 Falldog 修复了一个导致消息无限显示的问题，并新增了一项功能以减少发送到 Overpass Turbo 的请求数量！
- 已将 Overpass Turbo 实例切换为法国实例，相较于德国实例略快一些。

### v1.0.13 <time>2025-12-16</time>

- 感谢 _cyanliu_ 修正了便利店中的一个拼写错误，该错误导致图标无法显示！
